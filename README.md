...
graph TD
    ...
    subgraph CLIENTS ["Clients"]
        Student["🎓 Student<br/>Dashboard · Timetable · Planner"]
        Faculty["👤 Faculty / HOD<br/>Manage · Approvals · Notify"]
        Admin["⚙️ Admin<br/>Accounts · Audit · Broadcast"]
    end

    %% Next.js Layer
    subgraph NEXTJS ["Vercel — Next.js 14 (App Router)"]
        subgraph SC ["Server Components"]
            PagesLayouts["Pages + Layouts"]
            AuthGuard["Auth-guard + Sidebar inject"]
        end

        subgraph CC ["Client Components"]
            DashTTSub["Dashboard · Timetable · Subjects"]
            Hooks["useState · useEffect · useCallback"]
        end

        subgraph APIRoutes ["API Routes (server-side)"]
            GenDoc["generate-document"]
            CreateStaff["create-staff · deactivate-staff"]
            LogAction["log-action · check-login-attempt"]
        end

        Sidebar["Sidebar<br/>Role-aware · responsive"]
        DashTabs["DashboardTabs<br/>Overview · Profile · Audit"]
        PDFLib["pdf-lib<br/>Document gen"]
        RateLimit["rateLimit.ts<br/>DB-backed · per-action"]

        SupaClient["supabase/client.ts<br/>anon key · browser · RLS enforced"]
        SupaServer["supabase/server.ts<br/>service role key · API routes only"]
        ThreeR3F["Three.js + Drei<br/>3D landing page"]
    end

    %% Supabase Layer
    subgraph SUPABASE ["Supabase (managed PostgreSQL)"]
        subgraph Postgres ["PostgreSQL"]
            DB1["users · profiles · requests"]
            DB2["timetable_slots · exam_schedule"]
            DB3["subjects · student_subject_notes"]
            DB4["deadlines · notifications"]
            DB5["audit_log · rate_limits"]
            RLS["Row Level Security (RLS)<br/>pg_cron — 5 scheduled jobs"]
        end

        subgraph Auth ["Auth"]
            Auth1["Email + password · JWT tokens"]
            Auth2["PKCE password reset flow"]
            Auth3["skip_verification for staff"]
        end

        subgraph Storage ["Storage"]
            S1["avatars (public)"]
            S2["documents (public)"]
            S3["notifications (public)"]
            S4["request-attachments (private · signed URL)"]
        end
    end

    %% AI Microservice Layer
    subgraph AISERVICE ["Render — Python FastAPI (AI Microservice)"]
        subgraph Endpoints ["Endpoints"]
            E1["/parse-timetable"]
            E2["/parse-exam"]
            E3["/parse-almanac"]
            E4["/generate-plan"]
            CORS["CORS locked to Vercel origin"]
        end

        subgraph OCR ["OCR Pipeline"]
            OCR1["pdfplumber<br/>text-based PDFs"] --> OCR2["pdf2image<br/>render pages as PNG"]
            OCR2 --> OCR3["RapidOCR<br/>ONNX · pure Python"]
            OCR3 --> OCR4["fallback for scanned PDFs"]
        end

        subgraph Mistral ["Mistral API"]
            M1["httpx.post(MISTRAL_URL)<br/>no SDK · direct REST"]
            M2["Structured prompts<br/>return JSON arrays"]
            M3["Study planner prompt<br/>ranked priorities + reasoning"]
            M4["MISTRAL_API_KEY · MODEL · URL"]
        end
    end

    %% Connections
    CLIENTS -- "HTTPS" --> NEXTJS
    SupaClient -- "anon key" --> SUPABASE
    SupaServer -- "service role" --> SUPABASE
    NEXTJS -. "AI calls" .-> AISERVICE
```
