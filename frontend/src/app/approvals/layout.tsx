...
export default async function ApprovalsLayout({ children }: { children: React.ReactNode }) {
  ...
  // Students cannot access approvals
  if (!profile?.role || !['faculty', 'hod', 'admin'].includes(profile.role)) redirect('/dashboard')
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F2EDE6', fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
      <Sidebar fullName={profile?.full_name ?? 'Staff'} department={profile?.department ?? ''} year={profile?.year ?? null} section={profile?.section ?? null} role={profile.role} />
