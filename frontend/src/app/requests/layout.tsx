...
export default async function RequestsLayout({ children }: { children: React.ReactNode }) {
  ...
  // Staff go to approvals, not requests
  if (profile?.role && ['faculty', 'hod', 'admin'].includes(profile.role)) redirect('/approvals')
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F2EDE6', fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
      <Sidebar fullName={profile?.full_name ?? 'Student'} department={profile?.department ?? ''} year={profile?.year ?? null} section={profile?.section ?? null} role={profile?.role ?? 'student'} />
